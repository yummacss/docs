...
We're making some changes to Stylecent in v3 to make it more modern and consistent. These changes are turned on by default, but you can turn them off using the [`yumma.config.js`](#disabling-stylecent) file.
...
From now on, files like `yumma-core.css` and `yumma-core.min.css` will be deleted from the `/dist` folder in favor of the `yumma.config.js` config file.

<Tabs>
  <TabItem label="v3.0">
    ```js mark={4-7} title="yumma.config.js"
    module.exports = {
      source: ["./src/**/*.html"],
      output: "./src/globals.css",
      buildOptions: {
