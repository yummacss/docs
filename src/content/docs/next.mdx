...
You don't need to use `yummacss` in production since you're not depending on the package dependency files anymore.
...
<Steps>
    ...
    1. **Remove `@import` rules**
        ...
        ```
    			...
    			You can also install `yummacss` as a dev dependency, since you're not depending on the package directly.
...
We completely overhauled the codebase to get better performance in build times and overall file size. Rather, this new version involved a complete rewrite of the codebase, from the way utilities and modifiers are generated, to eliminate any potential for duplicated or unnecessary data in the `/dist` folder.
...
To make the Yumma CSS framework as complete as possible, we're adding support for more than 40 utility classes to the core of the framework.
