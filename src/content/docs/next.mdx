...
Don't include `yummacss` as a normal dependency if you're using the new command-line interface. We only need the CLI as a dev dependency since we're only using yummacss to compile our code to CSS.
