export const meta = {
	title: "Creating Apps",
	description: "Learn how to build custom apps with Yumma CSS Core.",
};
...
If you only need a specific category, you can import scoped functions. Some functions, like `colorUtils`, now accept a configuration object for custom color generation.

```js title="index.js"
import { backgroundUtils, colorUtils } from "@yummacss/core";

console.log(backgroundUtils()); // Returns background-related utilities

// Custom colors with 12% shade scaling
...
| `colorUtils()`         | Dynamic color palette based on user values.    |
...
> [!NOTE]
> `coreUtils` & `colorUtils` accept three optional parameters: `userColors` (object), `lightPercentage` (number), & `darkPercentage` (number).
...
```json frame="none" frame="terminal"
[
  { "prefix": "sm", "value": "min-width: 40rem" },
  { "prefix": "50", "value": "80" },
