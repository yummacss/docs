---
title: "@yummacss/cdn"
description: "Generate CSS in the browser, with no build step."
---

## Usage

```html title="index.html" mark={4}
<!doctype html>
<html>
  <head>
    <script src="https://unpkg.com/@yummacss/cdn"></script>
  </head>
  <body>
    <div class="c:indigo fw:500">Hello World</div>
  </body>
</html>
```

There is nothing else to set up. No config file, no marker in a stylesheet, no install step.

## When To Use It

You can use this for a single HTML file, a CodePen, or anything static where a build pipeline would be more trouble than it is worth, or if you just want to try things out, create quick design prototypes and test new ideas.

## Renamed From Runtime

This package was `@yummacss/runtime`. The old name still publishes the same script, so a page already pointing at it keeps working, but new pages should use `@yummacss/cdn`.
