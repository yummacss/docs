...
The `@yummacss/lint` tool reports any class in your project that it does not recognize and suggests a close match when one exists.
...
{
  "scripts": {
    "validate": "yummacss-lint"
  }
}
```

The binary is named `yummacss-lint`, not `lint`.
...
```bash
pnpm dlx @yummacss/lint -a docs-container -a brand-logo
```
...
Template literal expressions cannot be validated statically. In `` className={`p:4 ${size}`} `` the `p:4` is checked & the `${size}` is skipped, so classes assembled at runtime are invisible to it.
:::
...
Content files, such as a changelog, often contain classes that should not be validated. Pass a config inline through the API to scan only your application code:
...
Returns the set of class names found in a string, using the same class attribute contexts as `validate`. Useful for building project-specific rules on top of it:

```js
import { readFileSync } from "node:fs";
import { extractClasses } from "@yummacss/lint";
...
Canon needs nothing beyond your config & source files, so no build has to run first:

```yaml title=".github/workflows/ci.yml"
- uses: pnpm/action-setup@v6
- run: pnpm install
- run: pnpm dlx @yummacss/lint
```
