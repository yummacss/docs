...
The utility definitions themselves: every prefix, its CSS properties & its accepted values. Installed automatically as a dependency, so you only need it directly if you are building something that has to know what Yumma CSS can generate, such as an editor integration or a custom validator.
