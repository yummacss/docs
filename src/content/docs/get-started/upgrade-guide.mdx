...
{/* prettier-ignore */}
<div class="p-4">
  ```html mark={1} del={2} title="index.html" 
  <button class="bg-l-lead-6 h:bg-d-lead-1">Hello</button>
  <button class="bg-slate-1 h:bg-slate-7">Hello</button>
  ```
</div>
...
```html del={6-10} ins={1-5} title="index.html"
<div class="dir-b">...</div>
<div class="dir-i">...</div>
<div class="dir-l">...</div>
<div class="dir-r">...</div>
...
We're making some changes to Stylecent (base styles) in v3 to make it more modern and consistent. These changes are turned on by default, but you can turn them off using the [`yummacss.config.mjs`](#disabling-stylecent) file.
