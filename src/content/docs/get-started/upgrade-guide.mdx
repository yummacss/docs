...
+ <div class="h-dvh w-1/1">...</div>
```

This also applies to the Dimension utilities.
...
From now on, files like `yumma-core.css` and `yumma-core.min.css` will be deleted from the `/dist` folder in favor of the `yummacss.config.mjs` config file.

<Tabs>
  <TabItem label="v3.0">
    ```js mark={4-7} title="yummacss.config.mjs"
    export default {
      source: ["./src/**/*.html"],
