...
Only properties that accept a negative value take one: margins, positioning, z-index, letter spacing, text indent, order, grid lines, scroll margins, outline & underline offsets, rotate, scale, translate, skew & transition delay. On any other utility the class is refused, so `p:-4` & `w:-4` produce nothing.
...
The minus sign flips the sign of the utility's value, which matters for utilities whose value is already negative.
