export const meta = {
  title: "Upgrading",
  description: "Learn how to upgrade your project from Yumma CSS v2 to v3.",
};

Yumma CSS v3 is a major update with a long list of new features, along with some breaking changes that you'll need to know about when upgrading from v2 to v3.

## Working with the CLI

In v3, we've replaced the old `@import` and introduced a new CLI tool to work with Yumma CSS. Here's how to set it up:

<Stepper>
  <Step title="Update Yumma CSS">
    Remove the old `yummacss` package.

    ```bash title="Terminal"
    npm install yummacss@latest
    ```
  </Step>

  <Step title="Initialize configuration">
    Create a configuration file in your project.

    ```bash title="Terminal"
    npx yummacss init
    ```
  </Step>

  <Step title="Delete old imports">
    Remove the old `@import` statements from your CSS files.

    ```css title="styles.css" del={1-2}
    @import "yummacss/dist/yumma.min.css";
    @import "yummacss/dist/yumma.css";
    ```
  </Step>

  <Step title="Set up configuration">
    Specify the locations of all your project files in the config file.

    ```js title="yumma.config.mjs" mark={2-3}
    export default {
      source: ["./src/**/*.html"],
      output: "./src/styles.css",
      buildOptions: {
        reset: true,
        minify: false,
      },
    };
    ```
  </Step>

  <Step title="Build styles">
    You can now start generating your CSS with the [`build`](/docs/configuration/#build) or [`watch`](/docs/configuration/#watch) command.

    ```bash title="Terminal"
    npx yummacss build
    ```
  </Step>
</Stepper>

## Dependency changes

With Yumma CSS v3 you no longer need to install `yummacss` as a standard dependency. Instead, you can install it as a dev dependency. This is because Yumma CSS is now a CLI tool that generates CSS files based on your configuration.

```bash title="Terminal"
npm install yummacss -D
```

### Core Package

You'll need to replace the `@yummacss/api` package with the new but old same `@yummacss/core` and update your imports if you're using the `@yummacss/api` package directly.

```bash title="Terminal"
npm remove @yummacss/api
npm install @yummacss/core
```

## Utility changes

Here are the utilities that have been renamed in v3:

| Style               | v2      | v3     |
| ------------------- | ------- | ------ |
| `bottom`            | dir-b   | bo-\*  |
| `inset`             | dir-i   | i-\*   |
| `left`              | dir-l   | l-\*   |
| `right`             | dir-r   | r-\*   |
| `top`               | dir-t   | t-\*   |
| `columns`           | cols-* | c-*    |
| `height` & `width`  | dim-*  | d-*    |
| `font-size: 1rem;`  | fs-b    | fs-md  |
| `overflow`          | ovf-*  | o-*    |
| `float`             | flo-*  | fl-*   |
| `box-shadow`        | bs-*   | bsh-*  |
| `isolation`         | i-*    | is-*   |
| `rotate`            | t-r-*  | ro-*   |
| `system-ui`         | ff-s   | ff-d   |
| `charter`           | ff-c   | ff-s   |
| `height` / `width`  | *-1/1  | *-dvh  |
| _Any color utility_ | lead    | slate  |

| `*-lead`

### Color utility changes

In v3, both the light (`l-`) and dark (`d-`) characters are being removed across all color utilities. We are also renaming the `lead` color to `slate`.

<div className="p-4">
  ```html del={1} ins={2} title="index.html"
  <button class="bg-l-lead-6 h:bg-d-lead-1">Button</button>
  <button class="bg-slate-1 h:bg-slate-7">Button</button>
  ```
</div>

Also, the color hue is increasing from `10%` shade modification to `14%`. This means that light colors will become lighter, and dark colors will become darker.

| v2.1           | v3.0          | Difference    |
| -------------- | ------------- | ------------- |
| `*-l-indigo-6` | `*-indigo-1`  | -2 characters |
| `*-l-indigo-5` | `*-indigo-2`  | -2 characters |
| `*-l-indigo-4` | `*-indigo-3`  | -2 characters |
| `*-l-indigo-3` | `*-indigo-4`  | -2 characters |
| `*-l-indigo-2` | `*-indigo-5`  | -2 characters |
| `*-l-indigo-1` | `*-indigo-6`  | -2 characters |
| `*-indigo`     | `*-indigo`    | -0 characters |
| `*-d-indigo-1` | `*-indigo-7`  | -2 character  |
| `*-d-indigo-2` | `*-indigo-8`  | -2 character  |
| `*-d-indigo-3` | `*-indigo-9`  | -2 character  |
| `*-d-indigo-4` | `*-indigo-10` | -1 character  |
| `*-d-indigo-5` | `*-indigo-11` | -1 character  |
| `*-d-indigo-6` | `*-indigo-12` | -1 character  |

### Align and Justify

With Yumma CSS v3, the `align` and `justify` utilities will have the `stretch` utility suffix updated from `-s` to `-st`.

```html del={1-2} ins={3-4} title="index.html"
<div class="ai-st">...</div>
<div class="jc-st">...</div>
<div class="ai-s">...</div>
<div class="jc-s">...</div>
```

### Top / Right / Bottom / Left

The `bottom`, `inset`, `left`, `right`, and `top` utilities will have the `dir-` prefix removed.

```html del={1-5} ins={6-10} title="index.html"
<div class="dir-b-*">...</div>
<div class="dir-i-*">...</div>
<div class="dir-l-*">...</div>
<div class="dir-r-*">...</div>
<div class="dir-t-*">...</div>
<div class="bo-*">...</div>
<div class="i-*">...</div>
<div class="l-*">...</div>
<div class="r-*">...</div>
<div class="t-*">...</div>
```

### Columns

The `columns` utility have been shortened from `cols-*` to `c-*`.

```html del={1} ins={2} title="index.html"
<div class="cols-*">...</div>
<div class="c-*">...</div>
```

### Dimension

The `height` and `width` utilities have been shortened from `dim-*` to `d-*`.

```html del={1} ins={2} title="index.html"
<div class="dim-*">...</div>
<div class="d-*">...</div>
```

### Font Size

The `font-size` utilities have been renamed from `fs-b` to `fs-md`.

```html del={1} ins={2}  title="index.html"
<div class="fs-b">...</div>
<div class="fs-md">...</div>
```

### Font Family

The font family utility prefixes have been renamed to better reflect their purpose:

```html del={1-2} ins={3-4} title="index.html"
<div class="ff-s">...</div>
<div class="ff-c">...</div>
<div class="ff-d">...</div>
<div class="ff-s">...</div>
```

### Overflow

The `overflow` utilities have been shortened from `ovf-*` to `o-*`.

```html del={1} ins={2} title="index.html"
<div class="ovf-*">...</div>
<div class="o-*">...</div>
```

### Float

The `float` utilities have been shortened from `flo-*` to `fl-*`.

```html del={1} ins={2} title="index.html"
<div class="flo-*">...</div>
<div class="fl-*">...</div>
```

### Dynamic viewport

The `*-1/1` utility values have been renamed from `*-1/1` to `*-dvh`.

```html del={1-3} ins={4-6} title="index.html"
<div class="h-1/1">...</div>
<div class="w-1/1">...</div>
<div class="d-1/1">...</div>
<div class="h-dvh">...</div>
<div class="w-dvw">...</div>
<div class="d-dvh">...</div>
```

### Border Width utilities

The border width utilities have been renamed from `b-*` to `bw-*`:

```html del={1} ins={2} title="index.html"
<div class="b-*">...</div>
<div class="bw-*">...</div>
```

This also applies to directional border width utilities:

| Old Class | New Class |
| --------- | --------- |
| `bt-*`    | `btw-*`   |
| `br-*`    | `brw-*`   |
| `bb-*`    | `bbw-*`   |
| `bl-*`    | `blw-*`   |

### Border Style utilities

The border style utilities have been renamed from `b-*` to `bs-*`:

```html del={1} ins={2} title="index.html"
<div class="b-solid">...</div>
<div class="bs-solid">...</div>
```

### Border Radius utilities

The `rad-9` utility has been renamed to `br-pill`:

```html del={1} ins={2} title="index.html"
<div class="rad-9">...</div>
<div class="br-pill">...</div>
```

### Color utilities

The text color utility prefix has been shortened from `tc-` to `c-`:

```html del={1} ins={2} title="index.html"
<div class="tc-slate">...</div>
<div class="c-slate">...</div>
```

### Box Shadow

The box shadow utility prefix has been updated from `bs-` to `bsh-`:

```html del={1} ins={2} title="index.html"
<div class="bs-*">...</div>
<div class="bsh-*">...</div>
```

### Rotation

The rotation utility prefix has been updated from `t-r-` to `ro-` and now uses the standalone `rotate` property:

```html del={1} ins={2} title="index.html"
<div class="t-r-*">...</div>
<div class="ro-*">...</div>
```

### Isolation

The isolation utility prefix has been updated from `i-` to `is-`:

```html del={1} ins={2} title="index.html"
<div class="i-*">...</div>
<div class="is-*">...</div>
```

## Base style changes

We're making some changes to base styles of Yumma CSS in v3 to make it more modern and consistent. These changes are turned on by default, but you can turn them off using the [`yummacss.config.mjs`](#disabling-base-styles) file.

By default, all paddings will be removed.

```css mark={3} title="base.ts"
* {
  margin: 0;
  padding: 0;
}
```

Font rendering is smoother, and a consistent system font is set as the default. - [oshwcomeau.com](https://joshwcomeau.com/css/custom-css-reset)

```css mark={2} title="base.ts"
html {
  font-family: system-ui, sans-serif;
}
```

Form elements now include padding by default. Borders are added for form elements without class attributes.

```css ins={2-3} del={4} title="base.ts"
body {
  -webkit-font-smoothing: antialiased;
  font-family: system-ui, sans-serif;
  line-height: 1.5;
}
```

Form elements now include padding by default. Borders are added for form elements without class attributes.

```css mark={6-8,16} title="base.ts"
button,
input,
optgroup,
select,
textarea {
  background-color: transparent;
  font-family: inherit;
  padding: 0.5rem;
}

button:not([class]),
input:not([class]),
optgroup:not([class]),
select:not([class]),
textarea:not([class]) {
  border: 1px solid #bfc2c7;
}
```

Interactive elements have clear outlines for accessibility.

```css mark={7-9} title="base.ts"
button,
input,
textarea,
select,
a,
summary {
  &:focus {
    outline: 2px solid transparent;
  }
}
```

In the absence of content, textareas will exhibit a default height. - [piccalil.li](https://piccalil.li/blog/a-more-modern-css-reset)

```css mark={2} title="base.ts"
textarea:not([rows]) {
  min-height: 10em;
}
```

Disabled elements are visually distinct with reduced opacity and a "not-allowed" cursor.

```css mark={5-6} title="base.ts"
button:disabled,
input:disabled,
select:disabled,
textarea:disabled {
  cursor: not-allowed;
  opacity: 0.5;
}
```

Headings adopt balanced text wrapping, consistent font sizes, and bold weights. - [oshwcomeau.com](https://joshwcomeau.com/css/custom-css-reset)

```css mark={7-9,13} title="base.ts"
h1,
h2,
h3,
h4,
h5,
h6 {
  font-size: 1rem;
  font-weight: 600;
  text-wrap: balance;
}

p {
  text-wrap: pretty;
}
```

Small text and code elements are consistently scaled and inherited. Code elements will have consistent font family. - [modern-normalize](https://github.com/sindresorhus/modern-normalize/blob/main/modern-normalize.css#L64)

```css mark={3,7-8,15-16} title="base.ts"
b,
strong {
  font-weight: 700;
}

small {
  font-size: 80%;
  line-height: 1.4;
}

pre,
code,
kbd,
samp {
  font-family: monospace, monospace;
  font-size: 1em;
}
```

Reset default link styles.

```css mark={2,7} title="base.ts"
a {
  color: inherit;
  text-decoration: none;
}
```

Table headers are bold and sized consistently.

```css mark={2-3} title="base.ts"
th {
  font-size: 1rem;
  font-weight: 600;
}
```

Horizontal rules, details, and summaries are updated for better spacing and display. - [modern-normalize](https://github.com/sindresorhus/modern-normalize/blob/main/modern-normalize.css#L64)

```css mark={3,8,12} title="base.ts"
hr {
  border-top: 1px solid #bfc2c7;
  height: 0;
  margin: 1em 0;
}

details {
  display: block;
}

summary {
  display: list-item;
}
```

#### Disabling base styles

To disable the base styles generation you can set `buildOptions.reset` to `false` in the `yummacss.config.mjs` file.

```js mark={5} title="yummacss.config.mjs"
export default {
  source: ["./src/**/*.html"],
  output: "./src/out.css",
  buildOptions: {
    reset: false, // disables base styles
    minify: false,
  },
};
```
