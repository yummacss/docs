...
The `minify` build option is no longer available. Modern tools like Vite have become so fast that optimizing CSS internally in Yumma CSS is now obsolete. We recommend letting your bundler handle CSS minification.
...
Inherit the font family in `body` for better propagation.
