export const meta = {
  title: "Upgrading",
  description: "Upgrade your legacy project from Yumma CSS v2 to v3.",
};

Yumma CSS v3 is a major update with new features & breaking changes.

In v3, we replaced the old `@import` & introduced a new CLI tool.

<Stepper>
  <Step title="Update Yumma CSS">
    Remove the old `yummacss` package.

    ```bash title="Terminal"
    npm install yummacss@latest
    ```
  </Step>

  <Step title="Initialize configuration">
    Create a configuration file in your project.

    ```bash title="Terminal"
    npx yummacss init
    ```
  </Step>

  <Step title="Delete old imports">
    Remove the old `@import` statements from your CSS files.

    ```css title="styles.css" del={1-2}
    @import "yummacss/dist/yumma.min.css";
    @import "yummacss/dist/yumma.css";
    ```
  </Step>

  <Step title="Set up configuration">
    Specify the locations of all your project files in the config file.

    ```js title="yumma.config.mjs" mark={2-3}
    export default {
      source: ["./src/**/*.html"],
      output: "./src/styles.css",
      buildOptions: {
        reset: true,
        minify: false,
      },
    };
    ```
  </Step>

  <Step title="Build styles">
    You can now start generating your CSS with the [`build`](/docs/configuration/#build) or [`watch`](/docs/configuration/#watch) command.

    ```bash title="Terminal"
    npx yummacss build
    ```
  </Step>
</Stepper>

## Dependency changes

Yumma CSS v3 works as a CLI tool. Install it as a dev dependency (`-D`) instead of a standard dependency.

```bash title="Terminal"
npm install yummacss -D
```

### Core package

Replace the `@yummacss/api` package with `@yummacss/core`. Update your imports accordingly.

```bash title="Terminal"
npm remove @yummacss/api
npm install @yummacss/core
```

## Utility changes

Here are the utilities that have been renamed in v3:

| Style               | v2      | v3     |
| ------------------- | ------- | ------ |
| `bottom`            | dir-b   | bo-\*  |
| `inset`             | dir-i   | i-\*   |
| `left`              | dir-l   | l-\*   |
| `right`             | dir-r   | r-\*   |
| `top`               | dir-t   | t-\*   |
| `columns`           | cols-* | c-*    |
| `height` & `width`  | dim-*  | d-*    |
| `font-size: 1rem;`  | fs-b    | fs-md  |
| `overflow`          | ovf-*  | o-*    |
| `float`             | flo-*  | fl-*   |
| `box-shadow`        | bs-*   | bsh-*  |
| `isolation`         | i-*    | is-*   |
| `rotate`            | t-r-*  | ro-*   |
| `system-ui`         | ff-s   | ff-d   |
| `charter`           | ff-c   | ff-s   |
| `height` / `width`  | *-1/1  | *-dvh  |
| _Any color utility_ | lead    | slate  |

| `*-lead`

### Color utility changes

In v3, light (`l-`) & dark (`d-`) characters are removed across all color utilities. `lead` is renamed to `slate`.

<div className="p-4">
  ```html del={1} ins={2} title="index.html"
  <button class="bg-l-lead-6 h:bg-d-lead-1">Button</button>
  <button class="bg-slate-1 h:bg-slate-7">Button</button>
  ```
</div>

Color hue modification increased from 10 % to 14 %. Light colors are lighter; dark colors are darker.

| v2.1           | v3.0          | Difference    |
| -------------- | ------------- | ------------- |
| `*-l-indigo-6` | `*-indigo-1`  | -2 characters |
| `*-l-indigo-5` | `*-indigo-2`  | -2 characters |
| `*-l-indigo-4` | `*-indigo-3`  | -2 characters |
| `*-l-indigo-3` | `*-indigo-4`  | -2 characters |
| `*-l-indigo-2` | `*-indigo-5`  | -2 characters |
| `*-l-indigo-1` | `*-indigo-6`  | -2 characters |
| `*-indigo`     | `*-indigo`    | -0 characters |
| `*-d-indigo-1` | `*-indigo-7`  | -2 character  |
| `*-d-indigo-2` | `*-indigo-8`  | -2 character  |
| `*-d-indigo-3` | `*-indigo-9`  | -2 character  |
| `*-d-indigo-4` | `*-indigo-10` | -1 character  |
| `*-d-indigo-5` | `*-indigo-11` | -1 character  |
| `*-d-indigo-6` | `*-indigo-12` | -1 character  |

### Align & justify

Stretch utility suffixes updated from `-s` to `-st`.

```html del={1-2} ins={3-4} title="index.html"
<div class="ai-st">...</div>
<div class="jc-st">...</div>
<div class="ai-s">...</div>
<div class="jc-s">...</div>
```

### Top / Right / Bottom / Left

`dir-` prefix removed for layout utilities.

```html del={1-5} ins={6-10} title="index.html"
<div class="dir-b-*">...</div>
<div class="dir-i-*">...</div>
<div class="dir-l-*">...</div>
<div class="dir-r-*">...</div>
<div class="dir-t-*">...</div>
<div class="bo-*">...</div>
<div class="i-*">...</div>
<div class="l-*">...</div>
<div class="r-*">...</div>
<div class="t-*">...</div>
```

### Columns

`cols-*` shortened to `c-*`.

```html del={1} ins={2} title="index.html"
<div class="cols-*">...</div>
<div class="c-*">...</div>
```

### Dimension

`dim-*` shortened to `d-*`.

```html del={1} ins={2} title="index.html"
<div class="dim-*">...</div>
<div class="d-*">...</div>
```

### Font size

`fs-b` renamed to `fs-md`.

```html del={1} ins={2}  title="index.html"
<div class="fs-b">...</div>
<div class="fs-md">...</div>
```

### Font family

Utility prefixes renamed for clarity:

```html del={1-2} ins={3-4} title="index.html"
<div class="ff-s">...</div>
<div class="ff-c">...</div>
<div class="ff-d">...</div>
<div class="ff-s">...</div>
```

### Overflow

`ovf-*` shortened to `o-*`.

```html del={1} ins={2} title="index.html"
<div class="ovf-*">...</div>
<div class="o-*">...</div>
```

### Float

`flo-*` shortened to `fl-*`.

```html del={1} ins={2} title="index.html"
<div class="flo-*">...</div>
<div class="fl-*">...</div>
```

### Dynamic viewport

`*-1/1` renamed to `*-dvh`.

```html del={1-3} ins={4-6} title="index.html"
<div class="h-1/1">...</div>
<div class="w-1/1">...</div>
<div class="d-1/1">...</div>
<div class="h-dvh">...</div>
<div class="w-dvw">...</div>
<div class="d-dvh">...</div>
```

### Border width

`b-*` renamed to `bw-*`.

```html del={1} ins={2} title="index.html"
<div class="b-*">...</div>
<div class="bw-*">...</div>
```

This also applies to directional border width utilities:

| Old Class | New Class |
| --------- | --------- |
| `bt-*`    | `btw-*`   |
| `br-*`    | `brw-*`   |
| `bb-*`    | `bbw-*`   |
| `bl-*`    | `blw-*`   |

### Border style

`b-*` renamed to `bs-*`.

```html del={1} ins={2} title="index.html"
<div class="b-solid">...</div>
<div class="bs-solid">...</div>
```

### Border radius

`rad-9` renamed to `br-pill`.

```html del={1} ins={2} title="index.html"
<div class="rad-9">...</div>
<div class="br-pill">...</div>
```

### Color utilities

Text color prefix shortened from `tc-` to `c-`.

```html del={1} ins={2} title="index.html"
<div class="tc-slate">...</div>
<div class="c-slate">...</div>
```

### Box shadow

Prefix updated from `bs-` to `bsh-`.

```html del={1} ins={2} title="index.html"
<div class="bs-*">...</div>
<div class="bsh-*">...</div>
```

### Rotation

Prefix updated from `t-r-` to `ro-`. v3 uses the standalone `rotate` property.

```html del={1} ins={2} title="index.html"
<div class="t-r-*">...</div>
<div class="ro-*">...</div>
```

### Isolation

Prefix updated from `i-` to `is-`.

```html del={1} ins={2} title="index.html"
<div class="i-*">...</div>
<div class="is-*">...</div>
```

## Base style changes

Yumma CSS v3 modernizes base styles for consistency. These are enabled by default & can be disabled in `yumma.config.mjs`.

Remove paddings by default:

```css mark={3} title="base.ts"
* {
  margin: 0;
  padding: 0;
}
```

Smother font rendering with a default system font. Source: [joshwcomeau.com](https://joshwcomeau.com/css/custom-css-reset).

```css mark={2} title="base.ts"
html {
  font-family: system-ui, sans-serif;
}
```

Form elements now include padding by default. Borders are added for form elements without class attributes.

```css ins={2-3} del={4} title="base.ts"
body {
  -webkit-font-smoothing: antialiased;
  font-family: system-ui, sans-serif;
  line-height: 1.5;
}
```

Form elements now include padding by default. Borders are added for form elements without class attributes.

```css mark={6-8,16} title="base.ts"
button,
input,
optgroup,
select,
textarea {
  background-color: transparent;
  font-family: inherit;
  padding: 0.5 rem;
}

button:not([class]),
input:not([class]),
optgroup:not([class]),
select:not([class]),
textarea:not([class]) {
  border: 1px solid #bfc2c7;
}
```

Clear outlines for interactive elements to improve accessibility.

```css mark={7-9} title="base.ts"
button,
input,
textarea,
select,
a,
summary {
  &:focus {
    outline: 2px solid transparent;
  }
}
```

Textareas without `rows` use a default height. Source: [piccalil.li](https://piccalil.li/blog/a-more-modern-css-reset).

```css mark={2} title="base.ts"
textarea:not([rows]) {
  min-height: 10 em;
}
```

Distinct visual states for disabled elements.

```css mark={5-6} title="base.ts"
button:disabled,
input:disabled,
select:disabled,
textarea:disabled {
  cursor: not-allowed;
  opacity: 0.5;
}
```

Headings use balanced wrapping, consistent sizes, & bold weights.

```css mark={7-9,13} title="base.ts"
h1,
h2,
h3,
h4,
h5,
h6 {
  font-size: 1rem;
  font-weight: 600;
  text-wrap: balance;
}

p {
  text-wrap: pretty;
}
```

Standardized scaling for small text & code elements. Source: [modern-normalize](https://github.com/sindresorhus/modern-normalize).

```css mark={3,7-8,15-16} title="base.ts"
b,
strong {
  font-weight: 700;
}

small {
  font-size: 80 %;
  line-height: 1.4;
}

pre,
code,
kbd,
samp {
  font-family: monospace, monospace;
  font-size: 1 em;
}
```

Reset default link styles:

```css mark={2,7} title="base.ts"
a {
  color: inherit;
  text-decoration: none;
}
```

Consistent table header styling:

```css mark={2-3} title="base.ts"
th {
  font-size: 1rem;
  font-weight: 600;
}
```

Updated spacing for miscellaneous elements.

```css mark={3,8,12} title="base.ts"
hr {
  border-top: 1px solid #bfc2c7;
  height: 0;
  margin: 1 em 0;
}

details {
  display: block;
}

summary {
  display: list-item;
}
```

### Disable base styles

Set `buildOptions.reset` to `false` in `yumma.config.mjs` to disable base styles.

```js mark={5} title="yumma.config.mjs"
export default {
  buildOptions: {
    reset: false,
    minify: false,
  },
};
```
