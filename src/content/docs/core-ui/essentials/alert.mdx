---
title: Alert
description: A component that displays important messages and notifications to users.
slug: docs/essentials/alert
draft: true
---

## Examples

A basic implementation of the Alert component with default styling.

```tsx
import { Alert } from "yumma-ui";
import { InfoCircledIcon } from "@radix-ui/react-icons";

const App = () => {
  return (
    <Alert variant="base">
      <InfoCircledIcon />
      <AlertContent>
        <AlertTitle>Information</AlertTitle>
        <AlertDescription>This is an informational message to help guide users through the interface.</AlertDescription>
      </AlertContent>
    </Alert>
  );
};

export default App;
```

## Usage

```tsx
import { Alert } from "yumma-ui";
```

## API reference

The Alert component can be adapted to suit specific requirements by utilizing the internal API of Yumma UI, thereby enabling the modification of its visual appearance and operational characteristics.

### `variant`

> Default value is `base`

The `variant` property allows you to choose from different pre-defined styles that match your design system.

```ts
(property) variant?: "base" | "error" | "info" | "success" | "warning" | null | undefined
```

## Sub-components

The Alert component is composed of several sub-components that work together to create a structured alert message.

| Component          | Description                                                     | Props                                      |
| ------------------ | --------------------------------------------------------------- | ------------------------------------------ |
| `AlertContent`     | Wraps the alert's title and description content.                | Extends all native HTML `<div>` attributes |
| `AlertTitle`       | Displays the main heading or title of the alert message.        | Extends all native HTML `<h3>` attributes  |
| `AlertDescription` | Contains the detailed message or description text of the alert. | Extends all native HTML `<div>` attributes |

## Custom styling

The Alert component exposes a simple API to control its look and behavior through props.

```tsx
import { Alert } from "yumma-ui";

<Alert className="...">Example</Alert>;
```

## Extend properties

The Alert component is fully extendable. It supports all native HTML `<alert>` attributes through its props:

```tsx "onClick={() => alert("Alert clicked!")}"
import { Alert } from "yumma-ui";

const App = () => {
  return <Alert onClick={() => alert("Alert clicked!")}>Example</Alert>;
};

export default App;
```
