...
Two pseudo classes compile to a selector that requires both at once. `f:h:bg-red` produces `:focus:hover`, matching only an element that is focused *and* hovered in the same moment.

That is a different rule from styling either state, not a shorter way of writing it. Pick the one that says what you mean.

```tsx
<button type="button" className="f:h:bg-red">Focused and hovered</button>
<button type="button" className="f:bg-red h:bg-red">Focused or hovered</button>
```
