---
title: Align Items
banner:
  content: Support Yumma CSS by <a href=https://www.yummacss.com/docs/next>starring us on GitHub</a>!
description: Controls flex and grid positioning along a container's transverse axis.
slug: docs/align-items
---

<Utility
  data={[
    {
      className: "ai-b",
      properties: ["align-items: baseline;"],
    },
    {
      className: "ai-c",
      properties: ["align-items: center;"],
    },
    {
      className: "ai-fe",
      properties: ["align-items: flex-end;"],
    },
    {
      className: "ai-fs",
      properties: ["align-items: flex-start;"],
    },
    {
      className: "ai-s",
      properties: ["align-items: stretch;"],
    },
  ]}
/>

## Baseline

This example sets the align items to **baseline**. The items will be aligned along the baseline of the container.

```html live "ai-b" layout="/src/layouts/wide.astro"
<div class="ai-b stripes d-g g-4 gtc-3 h-64 tc-white ta-c w-full">
  <div class="bg-indigo ta-c p-4 rad-1">A</div>
  <div class="bg-indigo ta-c p-4 rad-1">B</div>
  <div class="bg-indigo ta-c p-4 rad-1">C</div>
  <div class="bg-indigo ta-c p-4 rad-1">D</div>
  <div class="bg-indigo ta-c p-4 rad-1">E</div>
</div>
```

## Center

This example sets the align items to **center**. The items will be centered within the container.

```html live "ai-c" layout="/src/layouts/wide.astro"
<div class="ai-c stripes d-g g-4 gtc-3 h-64 tc-white ta-c w-full">
  <div class="bg-indigo p-4 ta-c rad-1">A</div>
  <div class="bg-indigo p-4 ta-c rad-1">B</div>
  <div class="bg-indigo p-4 ta-c rad-1">C</div>
  <div class="bg-indigo p-4 ta-c rad-1">D</div>
  <div class="bg-indigo p-4 ta-c rad-1">E</div>
</div>
```

## Flex End

This example sets the align items to **flex-end**. The items will be aligned to the end of the container.

```html live "ai-fe" layout="/src/layouts/wide.astro"
<div class="ai-fe stripes d-g g-4 gtc-3 h-64 tc-white ta-c w-full">
  <div class="bg-indigo ta-c p-4 rad-1">A</div>
  <div class="bg-indigo ta-c p-4 rad-1">B</div>
  <div class="bg-indigo ta-c p-4 rad-1">C</div>
  <div class="bg-indigo ta-c p-4 rad-1">D</div>
  <div class="bg-indigo ta-c p-4 rad-1">E</div>
</div>
```

## Flex Start

This example sets the align items to **flex-start**. The items will be aligned to the start of the container.

```html live "ai-fs" layout="/src/layouts/wide.astro"
<div class="ai-fs stripes d-g g-4 gtc-3 h-64 tc-white ta-c w-full">
  <div class="bg-indigo ta-c p-4 rad-1">A</div>
  <div class="bg-indigo ta-c p-4 rad-1">B</div>
  <div class="bg-indigo ta-c p-4 rad-1">C</div>
  <div class="bg-indigo ta-c p-4 rad-1">D</div>
  <div class="bg-indigo ta-c p-4 rad-1">E</div>
</div>
```

## Stretch

This example sets the align items to **stretch**. The items will stretch to fill the container along the cross axis.

```html live "ai-s" layout="/src/layouts/wide.astro"
<div class="ai-s stripes d-g g-4 gtc-3 h-64 tc-white ta-c w-full">
  <div class="ai-c bg-indigo d-f jc-c p-4 rad-1">A</div>
  <div class="ai-c bg-indigo d-f jc-c p-4 rad-1">B</div>
  <div class="ai-c bg-indigo d-f jc-c p-4 rad-1">C</div>
  <div class="ai-c bg-indigo d-f jc-c p-4 rad-1">D</div>
  <div class="ai-c bg-indigo d-f jc-c p-4 rad-1">E</div>
</div>
```

<Modifier
  utility="ai-s"
  prefixUtil="ai"
  modifiers={{
    hoverUtil: "ai-e",
    responsiveUtil: "ai-e",
  }}
/>
