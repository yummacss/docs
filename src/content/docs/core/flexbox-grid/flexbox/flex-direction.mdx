---
title: Flex Direction
banner:
  content: Support Yumma CSS by <a href=https://www.yummacss.com/docs/next>starring us on GitHub</a>!
description: Controls the direction of flex elements.
slug: docs/flex-direction
---

<Utility
  data={[
    {
      className: "fd-c",
      properties: ["flex-direction: column;"],
    },
    {
      className: "fd-cr",
      properties: ["flex-direction: column-reverse;"],
    },
    {
      className: "fd-r",
      properties: ["flex-direction: row;"],
    },
    {
      className: "fd-rr",
      properties: ["flex-direction: row-reverse;"],
    },
  ]}
/>

## Column

This example sets the flex direction to **column**. The items will be arranged vertically in a single column.

```html live "fd-c" layout="/src/layouts/wide.astro"
<div class="p-r">
  <div class="fd-c s-y-4 tc-white">
    <div class="bg-indigo ta-c p-4 rad-1">A</div>
    <div class="bg-indigo ta-c p-4 rad-1">B</div>
    <div class="bg-indigo ta-c p-4 rad-1">C</div>
  </div>
</div>
```

## Column Reverse

This example sets the flex direction to **column-reverse**. The items will be arranged vertically in a single column, but in reverse order.

```html live "fd-cr" layout="/src/layouts/wide.astro"
<div class="p-r">
  <div class="fd-cr s-y-4 tc-white">
    <div class="bg-indigo ta-c p-4 rad-1">A</div>
    <div class="bg-indigo ta-c p-4 rad-1">B</div>
    <div class="bg-indigo ta-c p-4 rad-1">C</div>
  </div>
</div>
```

## Row

This example sets the flex direction to **row**. The items will be arranged horizontally in a single row.

```html live "fd-r" layout="/src/layouts/wide.astro"
<div class="p-r">
  <div class="cg-4 d-f fd-r tc-white">
    <div class="ai-c bg-indigo dim-14 d-f jc-c rad-1">A</div>
    <div class="ai-c bg-indigo dim-14 d-f jc-c rad-1">B</div>
    <div class="ai-c bg-indigo dim-14 d-f jc-c rad-1">C</div>
  </div>
</div>
```

## Row Reverse

This example sets the flex direction to **row-reverse**. The items will be arranged horizontally in a single row, but in reverse order.

```html live "fd-rr" layout="/src/layouts/wide.astro"
<div class="p-r">
  <div class="cg-4 d-f fd-rr tc-white">
    <div class="ai-c bg-indigo dim-14 d-f jc-c rad-1">A</div>
    <div class="ai-c bg-indigo dim-14 d-f jc-c rad-1">B</div>
    <div class="ai-c bg-indigo dim-14 d-f jc-c rad-1">C</div>
  </div>
</div>
```

<Modifier
  utility="fd-c"
  prefixUtil="fd"
  modifiers={{
    hoverUtil: "fd-r",
    responsiveUtil: "fd-r",
  }}
/>
