---
title: Border Collapse
banner:
  content: N/A
description: Controls the collapsing or separation of table borders.
slug: docs/border-collapse
---

<Class name="border-collapse" />

## Collapse

This example sets the border collapse to **collapse**. The borders of the table will be merged together.

```html live "bc-c" layout="/src/layouts/inline.astro"
<table class="b-1 bc-indigo bc-c b-s ta-c tc-indigo w-full">
  <tr>
    <td class="b-1 bc-indigo b-s p-2">A</td>
    <td class="b-1 bc-indigo b-s p-2">B</td>
  </tr>
  <tr>
    <td class="b-1 bc-indigo b-s p-2">C</td>
    <td class="b-1 bc-indigo b-s p-2">D</td>
  </tr>
</table>
```

## Separate

> Initial value

This example sets the border collapse to **separate**. The borders of the table cells will remain distinct, allowing for spacing between them.

```html live "bc-s" layout="/src/layouts/inline.astro"
<table class="b-1 bc-indigo bc-s b-s ta-c tc-indigo w-full">
  <tr>
    <td class="b-1 bc-indigo b-s p-2">A</td>
    <td class="b-1 bc-indigo b-s p-2">B</td>
  </tr>
  <tr>
    <td class="b-1 bc-indigo b-s p-2">C</td>
    <td class="b-1 bc-indigo b-s p-2">D</td>
  </tr>
</table>
```

## Conditional styles

Learn how to override existing utilities based on the user's [screen size](/docs/first-steps#breakpoint-modifiers) or other factors, such as [hover states](/docs/first-steps#hover-modifiers).

<MediaModifier class="bs-s" mediaModifier="bc-c" prefixClass="bs">
  ### Media modifier
</MediaModifier>

<HoverModifier class="bs-s" hoverModifier="bc-c" prefixClass="bs">
  ### Hover modifier
</HoverModifier>
