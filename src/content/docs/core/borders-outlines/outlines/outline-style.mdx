---
title: Outline Style
banner:
  content: N/A
description: Controls the outline style of an element.
slug: docs/outline-style
---

<Class name="outline-style" />

This example showcases various outline styles: **dashed**, **none** and **solid**.

- The **dashed** style creates a dashed outline around the element.
- The **none** style removes any outline from the element.
- The **solid** style creates a solid outline around the element.

```html live "os-none" "os-d" "os-s"
<div class="d-g g-16 gtc-1 sm:gtc-3">
  <div class="b-1 bc-indigo d-16 oc-indigo oo-1 os-none rad-1"></div>
  <div class="b-1 bc-indigo d-16 oc-indigo oo-1 os-d rad-1"></div>
  <div class="b-1 bc-indigo d-16 oc-indigo oo-1 os-s rad-1"></div>
</div>
```

## Conditional styles

<Modifier
  utility="os-h"
  prefixUtil="os"
  modifiers={{
    hoverMod: "os-s",
    responsiveMod: "os-s",
  }}
/>
