---
title: Outline Offset
banner:
  content: N/A
description: Controls the offset of an element's outline.
slug: docs/outline-offset
---

<Class name="outline-offset" />{" "}

This example showcases various outline offset values: **1px**, **2px** and **3px**.

- The **1px** offset places the outline 1 pixel away from the border.
- The **2px** offset places the outline 2 pixels away from the border.
- The **3px** offset places the outline 3 pixels away from the border.

```html live "oo-1" "oo-2" "oo-3"
<div class="d-g g-16 gtc-1 sm:gtc-3">
  <div class="b-1 bc-indigo d-16 oc-indigo oo-1 os-s rad-1"></div>
  <div class="b-1 bc-indigo d-16 oc-indigo oo-2 os-s rad-1"></div>
  <div class="b-1 bc-indigo d-16 oc-indigo oo-3 os-s rad-1"></div>
</div>
```

## Conditional styles

<Modifier
  utility="oo-1"
  prefixUtil="oo"
  modifiers={{
    hoverMod: "oo-2",
    responsiveMod: "oo-2",
  }}
/>
