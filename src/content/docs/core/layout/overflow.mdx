---
title: Overflow
banner:
  content: |
    Get ready for the <a href="https://www.yummacss.com/docs/next">next version of Yumma CSS</a>, which is going to be amazing!
description: Controls how an element behaves when content overflows
slug: docs/overflow
---

Controls how an element behaves when content overflows.

<Utility
  data={[
    {
      className: "ovf-auto",
      properties: ["overflow: auto;"],
    },
    {
      className: "ovf-c",
      properties: ["overflow: clip;"],
    },
    {
      className: "ovf-h",
      properties: ["overflow: hidden;"],
    },
    {
      className: "ovf-s",
      properties: ["overflow: scroll;"],
    },
    {
      className: "ovf-v",
      properties: ["overflow: visible;"],
    },
  ]}
/>

## Auto

This example sets the overflow to **auto**. The element will add scrollbars if the content overflows the container.

<Message icon="mouse">Try scrolling through the container to see how the overflow behaves.</Message>

```html live "ovf-auto"
<div class="dim-30 ovf-auto p-4 rad-2 stripes">
  <div class="ai-c bg-indigo d-f h-full jc-c rad-1 tc-white w-38">A</div>
</div>
```

## Clip

This example sets the overflow to **clip**. The content that overflows the container will be clipped and not visible.

```html live "ovf-c"
<div class="dim-30 ovf-c p-4 rad-2 stripes">
  <div class="ai-c bg-indigo d-f h-full jc-c rad-1 tc-white w-38">A</div>
</div>
```

## Hidden

This example sets the overflow to **hidden**. The content that overflows the container will be hidden and not visible, without scrollbars.

```html live "ovf-h"
<div class="dim-30 ovf-h p-4 rad-2 stripes">
  <div class="ai-c bg-indigo d-f h-full jc-c rad-1 tc-white w-38">A</div>
</div>
```

## Scroll

This example sets the overflow to **scroll**. The element will always show scrollbars, regardless of whether the content overflows.

```html live "ovf-s"
<div class="dim-30 ovf-s p-4 rad-2 stripes">
  <div class="ai-c bg-indigo d-f h-full jc-c rad-1 tc-white w-38">A</div>
</div>
```

## Visible

This example sets the overflow to **visible**. The content will overflow the container and be visible outside of it.

```html live "ovf-v"
<div class="dim-30 ovf-v p-4 rad-2 stripes">
  <div class="ai-c bg-indigo d-f h-full jc-c rad-1 tc-white w-38">A</div>
</div>
```

## Overflow X

Controls how an element behaves when content overflows on the X-axis.

<Utility
  data={[
    {
      className: "ovf-x-auto",
      properties: ["overflow-x: auto;"],
    },
    {
      className: "ovf-x-c",
      properties: ["overflow-x: clip;"],
    },
    {
      className: "ovf-x-h",
      properties: ["overflow-x: hidden;"],
    },
    {
      className: "ovf-x-s",
      properties: ["overflow-x: scroll;"],
    },
    {
      className: "ovf-x-v",
      properties: ["overflow-x: visible;"],
    },
  ]}
/>

## Overflow Y

Controls how an element behaves when content overflows on the Y-axis.

<Utility
  data={[
    {
      className: "ovf-y-auto",
      properties: ["overflow-y: auto;"],
    },
    {
      className: "ovf-y-c",
      properties: ["overflow-y: clip;"],
    },
    {
      className: "ovf-y-h",
      properties: ["overflow-y: hidden;"],
    },
    {
      className: "ovf-y-s",
      properties: ["overflow-y: scroll;"],
    },
    {
      className: "ovf-y-v",
      properties: ["overflow-y: visible;"],
    },
  ]}
/>

<Modifier
  utility="ovf-h"
  prefixUtil="ovf"
  modifiers={{
    hoverUtil: "ovf-auto",
    responsiveUtil: "ovf-auto",
  }}
/>
