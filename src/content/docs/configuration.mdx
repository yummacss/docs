...
A boolean that determines whether the inclusion of the [base styles](/docs/base-styles).
...
<button class="bg-white b-1 bc-silver-4">
  Button
</button>
```
...
<Note icon="info">
  Make sure you have a [`yumma.config.mjs`](/docs/config) file set up before running these commands. If you haven't, run `npx yummacss init` first.
</Note>
