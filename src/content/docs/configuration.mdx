...
To enable IntelliSense and type checking, add the following JSDoc comment at the top of your configuration file.

```mjs title="yumma.config.mjs" mark={1}
...
```mjs title="yumma.config.mjs" mark={2}
export default {
  source: ["./src/**/*.html"],
};
```
...
```mjs title="yumma.config.mjs" mark={3}
export default {
  buildOptions: {
