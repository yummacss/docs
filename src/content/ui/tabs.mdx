export const meta = {
  title: "Tabs",
  description: "Organize content into separate views where only one view can be visible at a time.",
};

## Basic Tabs

<Preview id="tabs-basic" />

```html
<div class="d-f fd-c g-4 w-full">
  <div
    role="tablist"
    aria-label="Content Categories"
    class="bg-silver-1 p-1 br-0 d-if bw-1 bc-silver-4"
  >
    <button
      type="button"
      role="tab"
      id="tab-for-you"
      aria-selected="true"
      aria-controls="panel-for-you"
      tabindex="0"
      class="fw-500 c-black bg-white px-4 py-2 br-0 bw-1 bc-silver-4 sh-sm"
    >
      For you
    </button>
    <button
      type="button"
      role="tab"
      id="tab-following"
      aria-selected="false"
      aria-controls="panel-following"
      tabindex="-1"
      class="fw-500 c-silver-10 px-4 py-2 h:c-black tr-c"
    >
      Following
    </button>
  </div>

  <div
    role="tabpanel"
    id="panel-for-you"
    aria-labelledby="tab-for-you"
    tabindex="0"
    class="p-4 bw-1 bc-silver-2 bg-white br-0 c-slate fs-sm d-b"
  >
    <p class="m-0">Content for the 'For you' tab. This section shows personalized recommendations based on your activity.</p>
  </div>

  <div
    role="tabpanel"
    id="panel-following"
    aria-labelledby="tab-following"
    hidden
    tabindex="0"
    class="p-4 bw-1 bc-silver-2 bg-white br-0 c-slate fs-sm d-none"
  >
    <p class="m-0">Content for the 'Following' tab. Stay updated with the latest posts from people you follow.</p>
  </div>
</div>
```

## Icons

<Preview id="tabs-icons" />

```html
<div class="d-f fd-c g-4 w-full">
  <div
    role="tablist"
    aria-label="View Modes"
    class="bg-silver-1 p-1 br-0 d-if bw-1 bc-silver-4"
  >
    <button
      type="button"
      role="tab"
      id="tab-grid"
      aria-selected="true"
      aria-controls="panel-grid"
      tabindex="0"
      class="d-f ai-c g-2 fw-500 c-black bg-white px-4 py-2 br-0 bw-1 bc-silver-4 sh-sm"
    >
      <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 256 256" aria-hidden="true">
        <rect width="256" height="256" fill="none" />
        <rect x="48" y="48" width="64" height="64" rx="8" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="16" />
        <rect x="144" y="48" width="64" height="64" rx="8" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="16" />
        <rect x="48" y="144" width="64" height="64" rx="8" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="16" />
        <rect x="144" y="144" width="64" height="64" rx="8" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="16" />
      </svg>
      <span>Grid</span>
    </button>
    <button
      type="button"
      role="tab"
      id="tab-list"
      aria-selected="false"
      aria-controls="panel-list"
      tabindex="-1"
      class="d-f ai-c g-2 fw-500 c-silver-10 px-4 py-2 h:c-black tr-c"
    >
      <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 256 256" aria-hidden="true">
        <rect width="256" height="256" fill="none" />
        <line x1="88" y1="64" x2="216" y2="64" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="16" />
        <line x1="88" y1="128" x2="216" y2="128" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="16" />
        <line x1="88" y1="192" x2="216" y2="192" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="16" />
        <circle cx="44" cy="64" r="12" fill="currentColor" />
        <circle cx="44" cy="128" r="12" fill="currentColor" />
        <circle cx="44" cy="192" r="12" fill="currentColor" />
      </svg>
      <span>List</span>
    </button>
  </div>

  <div
    role="tabpanel"
    id="panel-grid"
    aria-labelledby="tab-grid"
    tabindex="0"
    class="p-4 bw-1 bc-silver-2 bg-white br-0 c-slate fs-sm d-b"
  >
    <p class="m-0">Grid view content. Optimized for visual browsing of large collections.</p>
  </div>

  <div
    role="tabpanel"
    id="panel-list"
    aria-labelledby="tab-list"
    hidden
    tabindex="0"
    class="p-4 bw-1 bc-silver-2 bg-white br-0 c-slate fs-sm d-none"
  >
    <p class="m-0">List view content. Optimized for scanning detailed information quickly.</p>
  </div>
</div>
```

## Accessibility

The tabs component follows the [WAI-ARIA Tabs Pattern](https://www.w3.org/WAI/ARIA/apg/patterns/tabs/).

### Keyboard Interaction

| Key                          | Action                                                                                                                 |
|------------------------------|------------------------------------------------------------------------------------------------------------------------|
| `Tab`                        | Moves focus to the active tab. When focus is on the tab list, it moves to the next focusable element outside the list. |
| `Left Arrow` / `Right Arrow` | Moves focus between tabs and automatically activates the focused tab.                                                  |
| `Home`                       | Moves focus to the first tab and activates it.                                                                         |
| `End`                        | Moves focus to the last tab and activates it.                                                                          |

### Roles, States, and Properties

| Property          | Description                                                                 |
|-------------------|-----------------------------------------------------------------------------|
| `role="tablist"`  | Identifies the element as a container for a set of tabs.                    |
| `role="tab"`      | Identifies the element as a tab.                                            |
| `role="tabpanel"` | Identifies the element as a panel containing content associated with a tab. |
| `aria-selected`   | Indicates whether the tab is currently selected.                            |
| `aria-controls`   | References the ID of the tabpanel that the tab controls.                    |
| `aria-labelledby` | References the ID of the tab that labels the tabpanel.                      |
| `tabindex`        | Ensures that the active tab and tabpanel are focusable.                     |

