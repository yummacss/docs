...
Yumma CSS utilities are all single-class selectors, so they all have the same specificity, and the winner between two of them is whichever comes later **in the generated stylesheet**. Components pass your classes through [`merge`](/docs/class-merge), which drops the class yours replaces, so `c:accent` on a component that sets `c:white` takes effect.
