...
date: 2026-09-17
description: "Every utility is written with a colon now. One codemod, a new breakpoint & a reduced-motion variant."
title: Yumma CSS 4.0
---
...
`d:f` reads as `display: flex` with the whitespace taken out. `d-f` did not read as anything.
...
The rule is one line: `:` separates a property from its value, `-` subdivides a value. So a shade keeps its dash, because `red-5` is one value with a step, not two levels of nesting.
...
`none` & `auto` are never abbreviated, which two utilities did not follow. `tt-n` read as nothing at all.
...
`@yummacss/canon` is now [`@yummacss/lint`](/docs/lint), & its binary is `yummacss-lint`. Nothing about the API changed. `canon` named the idea, the set of classes Yumma CSS recognizes; `lint` names what you actually run.
