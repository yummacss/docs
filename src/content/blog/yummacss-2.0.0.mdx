---
authors:
- Renildo
cover: /blog/yummacss-2.0.0.png
date: 2024-08-12
description: "We're excited to announce the release of Yumma CSS v2.0.0! This new update is full of new features, big improvements, a redesigned source code, and of course, some important fixes."
title: Yumma CSS 2.0
---

## All-New Utilities

Continuous utility expansion remains a priority. This major release introduces several new tools for design control.

### Accent Color

```html "ac-(value)"
<input class="ac-red" type="checkbox" checked />
<input class="ac-orange" type="checkbox" checked />
<input class="ac-yellow" type="checkbox" checked />
```

### New Silver Color Palette

<Palette
  data={[
    {
      name: "Silver",
      color: "#bfc2c7",
    },
  ]}
/>

### Aspect Ratio

| Utility   | Property              |
| --------- | --------------------- |
| `ar-s`    | `aspect-ratio: 1/1;`  |
| `ar-l`    | `aspect-ratio: 16/9;` |
| `ar-auto` | `aspect-ratio: auto;` |

### Backdrop Filter

| Utility     | Property                       |
| ----------- | ------------------------------ |
| `bf-b-none` | `backdrop-filter: blur(0px);`  |
| `bf-b-xs`   | `backdrop-filter: blur(4px);`  |
| `bf-b-sm`   | `backdrop-filter: blur(8px);`  |
| `bf-b-md`   | `backdrop-filter: blur(16px);` |
| `bf-b-lg`   | `backdrop-filter: blur(32px);` |
| `bf-b-xl`   | `backdrop-filter: blur(64px);` |

### Border Collapse

| Utility | Property                     |
| ------- | ---------------------------- |
| `bc-c`  | `border-collapse: collapse;` |
| `bc-s`  | `border-collapse: separate;` |

### Border Style

| Utility  | Property                |
| -------- | ----------------------- |
| `b-none` | `border-style: none;`   |
| `b-d`    | `border-style: dashed;` |
| `b-h`    | `border-style: hidden;` |
| `b-s`    | `border-style: solid;`  |

### Caption Side

| Utility | Property                |
| ------- | ----------------------- |
| `cs-b`  | `caption-side: bottom;` |
| `cs-t`  | `caption-side: top;`    |

### Font Style

| Utility | Property              |
| ------- | --------------------- |
| `fs-i`  | `font-style: italic;` |
| `fs-n`  | `font-style: normal;` |

### Gap

Reworked the Gap utilities for more intuitive layout controls.

```html
<div class="g-[0/16]">…</div>
```

### Grid

```html "gc-s-(value) gce-(value) gcs-(value)" "gr-s-(value) gre-(value) grs-(value)"
<div class="gc-s-(value) gce-(value) gcs-(value)">…</div>
<div class="gr-s-(value) gre-(value) grs-(value)">…</div>
```

### Object Fit

| Utility   | Property                  |
| --------- | ------------------------- |
| `of-c`    | `object-fit: cover;`      |
| `of-f`    | `object-fit: fill;`       |
| `of-none` | `object-fit: none;`       |
| `of-sd`   | `object-fit: scale-down;` |

### Object Position

| Utility   | Property                         |
| --------- | -------------------------------- |
| `op-b`    | `object-position: bottom;`       |
| `op-c`    | `object-position: center;`       |
| `op-lb`   | `object-position: left bottom;`  |
| `op-left` | `object-position: left;`         |
| `op-lt`   | `object-position: left top;`     |
| `op-r`    | `object-position: right;`        |
| `op-rb`   | `object-position: right bottom;` |
| `op-rt`   | `object-position: right top;`    |
| `op-t`    | `object-position: top;`          |

### Outline Color

```html "oc-(value)"
<div class="o-4 oc-indigo-6 os-s"></div>
<div class="o-4 oc-violet-6 os-s"></div>
<div class="o-4 oc-pink-6 os-s"></div>
```

### Outline Offset

```html
<div class="oo-[0/4]">…</div>
```

### Outline Style

| Utility   | Property                 |
| --------- | ------------------------ |
| `os-none` | `outline-style: none;`   |
| `os-d`    | `outline-style: dashed;` |
| `os-h`    | `outline-style: hidden;` |
| `os-s`    | `outline-style: solid;`  |

### Outline Width

```html
<div class="ow-[0/4]">…</div>
```

### Spacing X

```html
<div class="s-x-(value)">…</div>
```

### Spacing Y

```html
<div class="s-y-(value)">…</div>
```

### Table Layout

| Utility   | Property               |
| --------- | ---------------------- |
| `tl-auto` | `table-layout: auto;`  |
| `tl-f`    | `table-layout: fixed;` |

### Text Decoration Color

```html "tdc-(value)"
<p class="tdc-red td-u">Red Underline</p>
<p class="tdc-green td-u">Green Underline</p>
<p class="tdc-blue td-u">Blue Underline</p>
```

### Text Decoration Style

| Utility    | Property                         |
| ---------- | -------------------------------- |
| `tds-none` | `text-decoration-style: none;`   |
| `tds-d`    | `text-decoration-style: dashed;` |
| `tds-h`    | `text-decoration-style: hidden;` |
| `tds-s`    | `text-decoration-style: solid;`  |

### Text Decoration Thickness

| Utility    | Property                                |
| ---------- | --------------------------------------- |
| `tdt-auto` | `text-decoration-thickness: auto;`      |
| `tdt-ff`   | `text-decoration-thickness: from-font;` |
| `tdt-0`    | `text-decoration-thickness: 0px;`       |
| `tdt-1`    | `text-decoration-thickness: 4px;`       |
| `tdt-2`    | `text-decoration-thickness: 8px;`       |
| `tdt-3`    | `text-decoration-thickness: 12px;`      |
| `tdt-4`    | `text-decoration-thickness: 16px;`      |

## Base Styles

Disable default base styles by importing `yumma-core.css` instead of `yumma.css`.

#### Standard CSS

```html
<link
  href="https://cdn.jsdelivr.net/gh/yumma-lib/yumma-css@2.0.0/dist/yumma-core.css"
  rel="stylesheet"
  crossorigin="anonymous"
/>
```

#### Minified CSS

```html
<link
  href="https://cdn.jsdelivr.net/gh/yumma-lib/yumma-css@2.0.0/dist/yumma-core.min.css"
  rel="stylesheet"
  crossorigin="anonymous"
/>
```

## Media Queries

Media query support is now available across nearly all Yumma CSS utilities for better responsiveness.

```html "sm:gtc-2" "md:gtc-3" "lg:gtc-4"
<div class="mx-auto p-4">
  <div class="d-g gtc-1 g-4 sm:gtc-2 md:gtc-3 lg:gtc-4">
    <div class="…">…</div>
    <div class="…">…</div>
    <div class="…">…</div>
  </div>
</div>
```

## Breakpoints Changes

Refined the breakpoint system by removing the `xs` breakpoint & adjusting existing values.

<Tabs defaultValue="v2.0">
    <TabsList>
        <TabsTab value="v2.0">v2.0</TabsTab>
        <TabsTab value="v1.2">v1.2</TabsTab>
    </TabsList>

<TabsPanel value="v2.0">
  | Utility | Property | |---------|--------------------| | `sm` | `min-width:
  640px;` | | `md` | `min-width: 768px;` | | `lg` | `min-width: 1024px;` | |
  `xl` | `min-width: 1280px;` | | `xxl` | `min-width: 1536px;` |
</TabsPanel>

  <TabsPanel value="v1.2">
| Utility | Property           |
|---------|--------------------|
| `xs`     | `min-width: 0;`     |
| `sm`     | `min-width: 480px;` |
| `md`     | `min-width: 720px;` |
| `lg`     | `min-width: 960px;` |
| `xl`     | `min-width: 1200px;` |
  </TabsPanel>
</Tabs>

## Container Rules

Introduced the `cnt` class for containers with built-in media query rules for responsiveness. It replaces v1.2's `cnn`, which was a fixed `1200px` maximum width with horizontal padding - `cnt` instead caps its width at each breakpoint.

The utility was later removed in [3.0](/blog/yummacss-3.0.0); see the [v2.0.0 release notes](https://github.com/yummacss/yummacss/releases/tag/v2.0.0) for the original implementation.

