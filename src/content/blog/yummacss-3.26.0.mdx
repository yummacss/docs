...
Everything keeps working. The CLI, `yumma.config.mjs`, & the `output` workflow work exactly as before — the plugins ignore `output` & inject the CSS through your bundler instead. Both dev servers also pick up edits to `yumma.config.mjs` without a restart.
