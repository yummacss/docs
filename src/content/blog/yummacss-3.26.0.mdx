...
Abbreviated utilities are easy to write, but they are also easy to get wrong - especially for AI models. LLMs love to mix in Tailwind syntax, writing `gap-4` when they mean `g-4`. The new `@yummacss/validator` package scans your source files & reports every class that Yumma CSS does not recognize.

```bash
npx yummacss-validator
```
...
Validity is checked against the Yumma CSS generator itself, so a class is valid exactly when it produces CSS - variants, opacity, negative values, custom theme colors, prefixes, & safelist entries are all understood. It exits with code `1` when unknown classes are found. Run it in CI to catch typos, or let your AI agent run it as a feedback loop & fix its own hallucinations. Use `--allow "my-custom-class"` for classes you define in your own CSS.
