/**
 * AUTO-GENERATED - do NOT edit this file manually!
 * Run: node scripts/generate-registry.mjs
 */

// ---------------------------------------------------------------------------
// Yumma UI Registry
// ---------------------------------------------------------------------------

export const registry = {
  "accordion": () => import("./ui/accordion"),
  "alert-dialog": () => import("./ui/alert-dialog"),
  "autocomplete-auto-highlight": () => import("./ui/autocomplete-auto-highlight"),
  "autocomplete-disabled": () => import("./ui/autocomplete-disabled"),
  "autocomplete-grouped": () => import("./ui/autocomplete-grouped"),
  "autocomplete-helper": () => import("./ui/autocomplete-helper"),
  "autocomplete-icon-leading": () => import("./ui/autocomplete-icon-leading"),
  "autocomplete-icon-trailing": () => import("./ui/autocomplete-icon-trailing"),
  "autocomplete-icon": () => import("./ui/autocomplete-icon"),
  "autocomplete-inset": () => import("./ui/autocomplete-inset"),
  "autocomplete-lg": () => import("./ui/autocomplete-lg"),
  "autocomplete-limit": () => import("./ui/autocomplete-limit"),
  "autocomplete-loading": () => import("./ui/autocomplete-loading"),
  "autocomplete-md": () => import("./ui/autocomplete-md"),
  "autocomplete-outset": () => import("./ui/autocomplete-outset"),
  "autocomplete-sm": () => import("./ui/autocomplete-sm"),
  "autocomplete-square": () => import("./ui/autocomplete-square"),
  "autocomplete-squircle": () => import("./ui/autocomplete-squircle"),
  "autocomplete-static": () => import("./ui/autocomplete-static"),
  "autocomplete": () => import("./ui/autocomplete"),
  "avatar-detailed": () => import("./ui/avatar-detailed"),
  "avatar-edit": () => import("./ui/avatar-edit"),
  "avatar-icon-fallback": () => import("./ui/avatar-icon-fallback"),
  "avatar-initial-fallback": () => import("./ui/avatar-initial-fallback"),
  "avatar-lg": () => import("./ui/avatar-lg"),
  "avatar-md": () => import("./ui/avatar-md"),
  "avatar-sm": () => import("./ui/avatar-sm"),
  "avatar-square": () => import("./ui/avatar-square"),
  "avatar-squircle": () => import("./ui/avatar-squircle"),
  "avatar-stack-compact": () => import("./ui/avatar-stack-compact"),
  "avatar-stacked": () => import("./ui/avatar-stacked"),
  "avatar-status": () => import("./ui/avatar-status"),
  "avatar-verified": () => import("./ui/avatar-verified"),
  "avatar": () => import("./ui/avatar"),
  "badge": () => import("./ui/badge"),
  "breadcrumb": () => import("./ui/breadcrumb"),
  "button-favorite": () => import("./ui/button-favorite"),
  "button-group-icon": () => import("./ui/button-group-icon"),
  "button-group-pill-label": () => import("./ui/button-group-pill-label"),
  "button-group-pill": () => import("./ui/button-group-pill"),
  "button-group": () => import("./ui/button-group"),
  "button-icon-leading": () => import("./ui/button-icon-leading"),
  "button-icon-trailing": () => import("./ui/button-icon-trailing"),
  "button": () => import("./ui/button"),
  "checkbox-group-label": () => import("./ui/checkbox-group-label"),
  "checkbox-group-nested-parent": () => import("./ui/checkbox-group-nested-parent"),
  "checkbox-parent": () => import("./ui/checkbox-parent"),
  "checkbox": () => import("./ui/checkbox"),
  "collapsible": () => import("./ui/collapsible"),
  "combobox-grouped": () => import("./ui/combobox-grouped"),
  "combobox-helper": () => import("./ui/combobox-helper"),
  "combobox": () => import("./ui/combobox"),
  "command-palette": () => import("./ui/command-palette"),
  "context-menu": () => import("./ui/context-menu"),
  "dialog-edit-profile": () => import("./ui/dialog-edit-profile"),
  "dialog-nested": () => import("./ui/dialog-nested"),
  "dialog-new-task": () => import("./ui/dialog-new-task"),
  "dialog-send-invite": () => import("./ui/dialog-send-invite"),
  "dialog-share-task": () => import("./ui/dialog-share-task"),
  "dialog-sign-in": () => import("./ui/dialog-sign-in"),
  "dialog-sign-up": () => import("./ui/dialog-sign-up"),
  "dialog": () => import("./ui/dialog"),
  "empty-state": () => import("./ui/empty-state"),
  "field-button": () => import("./ui/field-button"),
  "field-password": () => import("./ui/field-password"),
  "field-prefix": () => import("./ui/field-prefix"),
  "field-suffix": () => import("./ui/field-suffix"),
  "field": () => import("./ui/field"),
  "file-upload": () => import("./ui/file-upload"),
  "menu": () => import("./ui/menu"),
  "menubar": () => import("./ui/menubar"),
  "meter": () => import("./ui/meter"),
  "number-field": () => import("./ui/number-field"),
  "onboarding-checklist": () => import("./ui/onboarding-checklist"),
  "onboarding-pagination": () => import("./ui/onboarding-pagination"),
  "onboarding": () => import("./ui/onboarding"),
  "popover-color-palette": () => import("./ui/popover-color-palette"),
  "popover": () => import("./ui/popover"),
  "preview-card-compact": () => import("./ui/preview-card-compact"),
  "preview-card-multiple": () => import("./ui/preview-card-multiple"),
  "preview-card-project": () => import("./ui/preview-card-project"),
  "preview-card": () => import("./ui/preview-card"),
  "progress": () => import("./ui/progress"),
  "radio": () => import("./ui/radio"),
  "rating-emoji": () => import("./ui/rating-emoji"),
  "rating-feedback": () => import("./ui/rating-feedback"),
  "rating-readonly": () => import("./ui/rating-readonly"),
  "rating": () => import("./ui/rating"),
  "select-grouped": () => import("./ui/select-grouped"),
  "select": () => import("./ui/select"),
  "separator": () => import("./ui/separator"),
  "skeleton-activity": () => import("./ui/skeleton-activity"),
  "skeleton-filters": () => import("./ui/skeleton-filters"),
  "skeleton-inset": () => import("./ui/skeleton-inset"),
  "skeleton-list": () => import("./ui/skeleton-list"),
  "skeleton-outset": () => import("./ui/skeleton-outset"),
  "skeleton-static": () => import("./ui/skeleton-static"),
  "skeleton-stats": () => import("./ui/skeleton-stats"),
  "skeleton": () => import("./ui/skeleton"),
  "slider": () => import("./ui/slider"),
  "switch": () => import("./ui/switch"),
  "tabs": () => import("./ui/tabs"),
  "textarea": () => import("./ui/textarea"),
  "toggle-color-picker": () => import("./ui/toggle-color-picker"),
  "toggle-group": () => import("./ui/toggle-group"),
  "toggle": () => import("./ui/toggle"),
  "toolbar-input": () => import("./ui/toolbar-input"),
  "toolbar": () => import("./ui/toolbar"),
  "tooltip": () => import("./ui/tooltip"),
} as const;

export type RegistryId = keyof typeof registry;
export type RegistryImport = () => Promise<{ default: React.ComponentType<object> }>;

export function getRegistryImport(id: string): RegistryImport | null {
  return (registry as Record<string, RegistryImport>)[id] ?? null;
}

// ---------------------------------------------------------------------------
// Prop schemas, for components that have a real API
// ---------------------------------------------------------------------------

export interface RegistryProp {
  name: string;
  /**
   * The kind of control, not the TypeScript type. `none` covers everything a
   * JSON schema cannot offer a control for - a ReactNode, an array of objects -
   * which the props table still documents via `typeName`.
   */
  type: "enum" | "boolean" | "string" | "number" | "none";
  /** Shown in the Type column instead of `type`, when the two differ. */
  typeName?: string;
  values?: string[];
  default?: string | boolean | number;
  /**
   * A representative value for the demo, for a prop the component cannot
   * sensibly default. `default` stays the truth the props table reports.
   */
  example?: unknown;
  /**
   * An iconoir icon name, for a `ReactNode` prop that JSON cannot express.
   * Without this a component whose only visible content is an icon - Popover's
   * trigger, Toggle's face - demos itself as an empty box. The preview resolves
   * it to a real element & the usage snippet prints it as JSX plus its import,
   * so the copied code compiles. Names must exist in `EXAMPLE_ICONS` in
   * `component-preview.tsx`, a curated map rather than a dynamic lookup so the
   * bundler can still shake the icon set.
   */
  exampleIcon?: string;
  description?: string;
}

export interface RegistryMeta {
  summary?: string;
  props: RegistryProp[];
  /** Default text for components that take children. Absent means they do not. */
  children?: string;
}

export const registryMeta = {
  "accordion": () => import("./meta/accordion.json"),
  "alert-dialog": () => import("./meta/alert-dialog.json"),
  "autocomplete": () => import("./meta/autocomplete.json"),
  "avatar": () => import("./meta/avatar.json"),
  "badge": () => import("./meta/badge.json"),
  "breadcrumb": () => import("./meta/breadcrumb.json"),
  "button": () => import("./meta/button.json"),
  "checkbox": () => import("./meta/checkbox.json"),
  "collapsible": () => import("./meta/collapsible.json"),
  "combobox": () => import("./meta/combobox.json"),
  "command-palette": () => import("./meta/command-palette.json"),
  "context-menu": () => import("./meta/context-menu.json"),
  "dialog": () => import("./meta/dialog.json"),
  "empty-state": () => import("./meta/empty-state.json"),
  "field": () => import("./meta/field.json"),
  "file-upload": () => import("./meta/file-upload.json"),
  "menu": () => import("./meta/menu.json"),
  "menubar": () => import("./meta/menubar.json"),
  "meter": () => import("./meta/meter.json"),
  "number-field": () => import("./meta/number-field.json"),
  "onboarding": () => import("./meta/onboarding.json"),
  "popover": () => import("./meta/popover.json"),
  "preview-card": () => import("./meta/preview-card.json"),
  "progress": () => import("./meta/progress.json"),
  "radio": () => import("./meta/radio.json"),
  "rating": () => import("./meta/rating.json"),
  "select": () => import("./meta/select.json"),
  "separator": () => import("./meta/separator.json"),
  "skeleton": () => import("./meta/skeleton.json"),
  "slider": () => import("./meta/slider.json"),
  "switch": () => import("./meta/switch.json"),
  "tabs": () => import("./meta/tabs.json"),
  "textarea": () => import("./meta/textarea.json"),
  "toggle": () => import("./meta/toggle.json"),
  "toolbar": () => import("./meta/toolbar.json"),
  "tooltip": () => import("./meta/tooltip.json"),
} as const;

export type MetaImport = () => Promise<{ default: RegistryMeta }>;

export function getRegistryMeta(id: string): MetaImport | null {
  return (registryMeta as Record<string, MetaImport>)[id] ?? null;
}

// ---------------------------------------------------------------------------
// What `yummaui add` addresses
// ---------------------------------------------------------------------------

export interface RegistryTarget {
  component: string;
  /** `"base"` for the component itself. */
  variant: string;
  /**
   * `component` is the props-driven unit `add <name>` installs. `block` is a
   * composition installed under its own id. `example` only demonstrates a
   * prop, so there is nothing to install - you pass that prop instead.
   */
  kind: "component" | "block" | "example";
  /** The name to pass to `yummaui add`. */
  install: string;
}

export const registryTargets: Record<string, RegistryTarget> = {
  "accordion": { component: "accordion", variant: "base", kind: "component", install: "accordion" },
  "alert-dialog": { component: "alert-dialog", variant: "base", kind: "component", install: "alert-dialog" },
  "autocomplete-auto-highlight": { component: "autocomplete", variant: "auto-highlight", kind: "example", install: "autocomplete" },
  "autocomplete-disabled": { component: "autocomplete", variant: "disabled", kind: "example", install: "autocomplete" },
  "autocomplete-grouped": { component: "autocomplete", variant: "grouped", kind: "example", install: "autocomplete" },
  "autocomplete-helper": { component: "autocomplete", variant: "helper", kind: "example", install: "autocomplete" },
  "autocomplete-icon-leading": { component: "autocomplete", variant: "icon-leading", kind: "example", install: "autocomplete" },
  "autocomplete-icon-trailing": { component: "autocomplete", variant: "icon-trailing", kind: "example", install: "autocomplete" },
  "autocomplete-icon": { component: "autocomplete", variant: "icon", kind: "example", install: "autocomplete" },
  "autocomplete-inset": { component: "autocomplete", variant: "inset", kind: "example", install: "autocomplete" },
  "autocomplete-lg": { component: "autocomplete", variant: "lg", kind: "example", install: "autocomplete" },
  "autocomplete-limit": { component: "autocomplete", variant: "limit", kind: "example", install: "autocomplete" },
  "autocomplete-loading": { component: "autocomplete", variant: "loading", kind: "block", install: "autocomplete-loading" },
  "autocomplete-md": { component: "autocomplete", variant: "md", kind: "example", install: "autocomplete" },
  "autocomplete-outset": { component: "autocomplete", variant: "outset", kind: "example", install: "autocomplete" },
  "autocomplete-sm": { component: "autocomplete", variant: "sm", kind: "example", install: "autocomplete" },
  "autocomplete-square": { component: "autocomplete", variant: "square", kind: "example", install: "autocomplete" },
  "autocomplete-squircle": { component: "autocomplete", variant: "squircle", kind: "example", install: "autocomplete" },
  "autocomplete-static": { component: "autocomplete", variant: "static", kind: "example", install: "autocomplete" },
  "autocomplete": { component: "autocomplete", variant: "base", kind: "component", install: "autocomplete" },
  "avatar-detailed": { component: "avatar", variant: "detailed", kind: "example", install: "avatar" },
  "avatar-edit": { component: "avatar", variant: "edit", kind: "block", install: "avatar-edit" },
  "avatar-icon-fallback": { component: "avatar", variant: "icon-fallback", kind: "example", install: "avatar" },
  "avatar-initial-fallback": { component: "avatar", variant: "initial-fallback", kind: "example", install: "avatar" },
  "avatar-lg": { component: "avatar", variant: "lg", kind: "example", install: "avatar" },
  "avatar-md": { component: "avatar", variant: "md", kind: "example", install: "avatar" },
  "avatar-sm": { component: "avatar", variant: "sm", kind: "example", install: "avatar" },
  "avatar-square": { component: "avatar", variant: "square", kind: "example", install: "avatar" },
  "avatar-squircle": { component: "avatar", variant: "squircle", kind: "example", install: "avatar" },
  "avatar-stack-compact": { component: "avatar", variant: "stack-compact", kind: "example", install: "avatar" },
  "avatar-stacked": { component: "avatar", variant: "stacked", kind: "block", install: "avatar-stacked" },
  "avatar-status": { component: "avatar", variant: "status", kind: "example", install: "avatar" },
  "avatar-verified": { component: "avatar", variant: "verified", kind: "example", install: "avatar" },
  "avatar": { component: "avatar", variant: "base", kind: "component", install: "avatar" },
  "badge": { component: "badge", variant: "base", kind: "component", install: "badge" },
  "breadcrumb": { component: "breadcrumb", variant: "base", kind: "component", install: "breadcrumb" },
  "button-favorite": { component: "button", variant: "favorite", kind: "example", install: "button" },
  "button-group-icon": { component: "button", variant: "group-icon", kind: "block", install: "button-group-icon" },
  "button-group-pill-label": { component: "button", variant: "group-pill-label", kind: "block", install: "button-group-pill-label" },
  "button-group-pill": { component: "button", variant: "group-pill", kind: "block", install: "button-group-pill" },
  "button-group": { component: "button", variant: "group", kind: "block", install: "button-group" },
  "button-icon-leading": { component: "button", variant: "icon-leading", kind: "example", install: "button" },
  "button-icon-trailing": { component: "button", variant: "icon-trailing", kind: "example", install: "button" },
  "button": { component: "button", variant: "base", kind: "component", install: "button" },
  "checkbox-group-label": { component: "checkbox", variant: "group-label", kind: "block", install: "checkbox-group-label" },
  "checkbox-group-nested-parent": { component: "checkbox", variant: "group-nested-parent", kind: "block", install: "checkbox-group-nested-parent" },
  "checkbox-parent": { component: "checkbox", variant: "parent", kind: "block", install: "checkbox-parent" },
  "checkbox": { component: "checkbox", variant: "base", kind: "component", install: "checkbox" },
  "collapsible": { component: "collapsible", variant: "base", kind: "component", install: "collapsible" },
  "combobox-grouped": { component: "combobox", variant: "grouped", kind: "example", install: "combobox" },
  "combobox-helper": { component: "combobox", variant: "helper", kind: "example", install: "combobox" },
  "combobox": { component: "combobox", variant: "base", kind: "component", install: "combobox" },
  "command-palette": { component: "command-palette", variant: "base", kind: "component", install: "command-palette" },
  "context-menu": { component: "context-menu", variant: "base", kind: "component", install: "context-menu" },
  "dialog-edit-profile": { component: "dialog", variant: "edit-profile", kind: "block", install: "dialog-edit-profile" },
  "dialog-nested": { component: "dialog", variant: "nested", kind: "block", install: "dialog-nested" },
  "dialog-new-task": { component: "dialog", variant: "new-task", kind: "block", install: "dialog-new-task" },
  "dialog-send-invite": { component: "dialog", variant: "send-invite", kind: "block", install: "dialog-send-invite" },
  "dialog-share-task": { component: "dialog", variant: "share-task", kind: "block", install: "dialog-share-task" },
  "dialog-sign-in": { component: "dialog", variant: "sign-in", kind: "block", install: "dialog-sign-in" },
  "dialog-sign-up": { component: "dialog", variant: "sign-up", kind: "block", install: "dialog-sign-up" },
  "dialog": { component: "dialog", variant: "base", kind: "component", install: "dialog" },
  "empty-state": { component: "empty-state", variant: "base", kind: "component", install: "empty-state" },
  "field-button": { component: "field", variant: "button", kind: "block", install: "field-button" },
  "field-password": { component: "field", variant: "password", kind: "block", install: "field-password" },
  "field-prefix": { component: "field", variant: "prefix", kind: "example", install: "field" },
  "field-suffix": { component: "field", variant: "suffix", kind: "example", install: "field" },
  "field": { component: "field", variant: "base", kind: "component", install: "field" },
  "file-upload": { component: "file-upload", variant: "base", kind: "component", install: "file-upload" },
  "menu": { component: "menu", variant: "base", kind: "component", install: "menu" },
  "menubar": { component: "menubar", variant: "base", kind: "component", install: "menubar" },
  "meter": { component: "meter", variant: "base", kind: "component", install: "meter" },
  "number-field": { component: "number-field", variant: "base", kind: "component", install: "number-field" },
  "onboarding-checklist": { component: "onboarding", variant: "checklist", kind: "example", install: "onboarding" },
  "onboarding-pagination": { component: "onboarding", variant: "pagination", kind: "example", install: "onboarding" },
  "onboarding": { component: "onboarding", variant: "base", kind: "component", install: "onboarding" },
  "popover-color-palette": { component: "popover", variant: "color-palette", kind: "block", install: "popover-color-palette" },
  "popover": { component: "popover", variant: "base", kind: "component", install: "popover" },
  "preview-card-compact": { component: "preview-card", variant: "compact", kind: "block", install: "preview-card-compact" },
  "preview-card-multiple": { component: "preview-card", variant: "multiple", kind: "block", install: "preview-card-multiple" },
  "preview-card-project": { component: "preview-card", variant: "project", kind: "example", install: "preview-card" },
  "preview-card": { component: "preview-card", variant: "base", kind: "component", install: "preview-card" },
  "progress": { component: "progress", variant: "base", kind: "component", install: "progress" },
  "radio": { component: "radio", variant: "base", kind: "component", install: "radio" },
  "rating-emoji": { component: "rating", variant: "emoji", kind: "example", install: "rating" },
  "rating-feedback": { component: "rating", variant: "feedback", kind: "block", install: "rating-feedback" },
  "rating-readonly": { component: "rating", variant: "readonly", kind: "example", install: "rating" },
  "rating": { component: "rating", variant: "base", kind: "component", install: "rating" },
  "select-grouped": { component: "select", variant: "grouped", kind: "example", install: "select" },
  "select": { component: "select", variant: "base", kind: "component", install: "select" },
  "separator": { component: "separator", variant: "base", kind: "component", install: "separator" },
  "skeleton-activity": { component: "skeleton", variant: "activity", kind: "example", install: "skeleton" },
  "skeleton-filters": { component: "skeleton", variant: "filters", kind: "example", install: "skeleton" },
  "skeleton-inset": { component: "skeleton", variant: "inset", kind: "example", install: "skeleton" },
  "skeleton-list": { component: "skeleton", variant: "list", kind: "example", install: "skeleton" },
  "skeleton-outset": { component: "skeleton", variant: "outset", kind: "example", install: "skeleton" },
  "skeleton-static": { component: "skeleton", variant: "static", kind: "example", install: "skeleton" },
  "skeleton-stats": { component: "skeleton", variant: "stats", kind: "example", install: "skeleton" },
  "skeleton": { component: "skeleton", variant: "base", kind: "component", install: "skeleton" },
  "slider": { component: "slider", variant: "base", kind: "component", install: "slider" },
  "switch": { component: "switch", variant: "base", kind: "component", install: "switch" },
  "tabs": { component: "tabs", variant: "base", kind: "component", install: "tabs" },
  "textarea": { component: "textarea", variant: "base", kind: "component", install: "textarea" },
  "toggle-color-picker": { component: "toggle", variant: "color-picker", kind: "block", install: "toggle-color-picker" },
  "toggle-group": { component: "toggle", variant: "group", kind: "block", install: "toggle-group" },
  "toggle": { component: "toggle", variant: "base", kind: "component", install: "toggle" },
  "toolbar-input": { component: "toolbar", variant: "input", kind: "example", install: "toolbar" },
  "toolbar": { component: "toolbar", variant: "base", kind: "component", install: "toolbar" },
  "tooltip": { component: "tooltip", variant: "base", kind: "component", install: "tooltip" },
};

export function getRegistryTarget(id: string): RegistryTarget {
  return (
    registryTargets[id] ?? {
      component: id,
      variant: "base",
      kind: "component",
      install: id,
    }
  );
}
