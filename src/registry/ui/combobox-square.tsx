...
export default function ComboboxSquare() {
  const [open, setOpen] = useState(false);

  return (
    <Combobox.Root items={users} open={open} onOpenChange={setOpen}>
      <div className="d-f p-r fd-c g-2 c-slate-10 fs-sm">
        <label htmlFor="user-square-input" className="fw-500">
          Assign member
        </label>
        <div className="p-r">
          <Combobox.Input
            id="user-square-input"
            placeholder="Search team members"
            className="h-10 w-64 pl-4 pr-16 bg-white bc-silver-3 c-slate-10 bw-1 fs-md bs-o-xs fv:oo--1 fv:oc-indigo-5"
          />
          <div className="d-f p-a r-2 b-0 ai-c jc-c h-10 c-slate-6">
            <Combobox.Clear
              className="d-f b-0 ai-c jc-c w-6 h-6 p-0 bg-transparent c-slate-6 c-p h:c-slate-10 fv:oo--1 fv:oc-indigo-5"
        ...
        {open && (
          <Combobox.Portal keepMounted>
            <Combobox.Positioner className="ow-0" sideOffset={8}>
              ...
              >
                <Combobox.Popup className="o-h w-64 bg-white bc-silver-2 c-slate-10 bw-1 bs-o-xs">
                  <Combobox.List className="oy-auto py-1 max-h-72 ow-0">
                    {(user: User) => (
                      ...
                      >
                        <Avatar.Root className="d-if o-h ai-c jc-c w-6 h-6 bc-white bw-1 us-none">
                          <Avatar.Image
                            src={user.avatar}
                            alt={user.name}
