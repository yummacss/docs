export default function PricingSectionBasic() {
  return (
    <section className="py-12 px-6">
      <div className="ta-c mb-10">
        <h2 className="ff-s fs-3xl fw-600 c-slate-10 mb-2">
          Simple, transparent pricing
        </h2>
        <p className="fs-md c-silver-10 m-0">
          Choose the plan that works best for you
        </p>
      </div>
      <div className="d-f fw-w jc-c g-6">
        <div className="w-72 bw-1 bc-silver-4 br-0 p-6 d-f fd-c">
          <div className="mb-6">
            <h3 className="ff-s fs-lg fw-600 c-slate-10 mb-1">Free</h3>
            <p className="fs-sm c-silver-10 m-0">
              For individuals getting started
            </p>
          </div>
          <div className="mb-6">
            <span className="fs-4xl fw-600 c-slate-10">€0</span>
            <span className="fs-sm c-silver-10">/month</span>
          </div>
          <ul className="d-f fd-c g-3 mb-6 p-0">
            <li className="d-f ai-c g-2">
              <svg
                className="w-4 h-4 c-green f-current"
                viewBox="0 0 256 256"
                role="img"
                aria-label="Check"
              >
                <path d="M229.66,77.66l-128,128a8,8,0,0,1-11.32,0l-56-56a8,8,0,0,1,11.32-11.32L96,188.69,218.34,66.34a8,8,0,0,1,11.32,11.32Z" />
              </svg>
              <span className="fs-sm c-slate-8">Up to 3 projects</span>
            </li>
            <li className="d-f ai-c g-2">
              <svg
                className="w-4 h-4 c-green f-current"
                viewBox="0 0 256 256"
                role="img"
                aria-label="Check"
              >
                <path d="M229.66,77.66l-128,128a8,8,0,0,1-11.32,0l-56-56a8,8,0,0,1,11.32-11.32L96,188.69,218.34,66.34a8,8,0,0,1,11.32,11.32Z" />
              </svg>
              <span className="fs-sm c-slate-8">Basic analytics</span>
            </li>
            <li className="d-f ai-c g-2">
              <svg
                className="w-4 h-4 c-green f-current"
                viewBox="0 0 256 256"
                role="img"
                aria-label="Check"
              >
                <path d="M229.66,77.66l-128,128a8,8,0,0,1-11.32,0l-56-56a8,8,0,0,1,11.32-11.32L96,188.69,218.34,66.34a8,8,0,0,1,11.32,11.32Z" />
              </svg>
              <span className="fs-sm c-slate-8">Community support</span>
            </li>
          </ul>
          <button
            type="button"
            className="mt-auto ai-c d-f jc-c px-4 py-2 br-0 fs-sm fw-500 bg-white c-slate-8 bw-1 bc-silver-4 h:bg-silver-1 f:oc-silver-1 f:os-s f:ow-2"
          >
            Get started
          </button>
        </div>
        <div className="p-r w-72 bw-2 bc-slate-8 br-0 p-6 d-f fd-c">
          <div className="p-a t-0 l-0 r-0 h-0 d-f jc-c ai-c">
            <span className="bg-slate-9 c-white px-2 py-1 br-0 fs-xs fw-500">
              Popular
            </span>
          </div>
          <div className="mb-6">
            <h3 className="ff-s fs-lg fw-600 c-slate-10 m-0">Premium</h3>
            <p className="fs-sm c-silver-10 m-0">
              For growing teams and businesses
            </p>
          </div>
          <div className="mb-6">
            <span className="fs-4xl fw-600 c-slate-10">€59</span>
            <span className="fs-sm c-silver-10">/month</span>
          </div>
          <ul className="d-f fd-c g-3 mb-6 p-0">
            <li className="d-f ai-c g-2">
              <svg
                className="w-4 h-4 c-green f-current"
                viewBox="0 0 256 256"
                role="img"
                aria-label="Check"
              >
                <path d="M229.66,77.66l-128,128a8,8,0,0,1-11.32,0l-56-56a8,8,0,0,1,11.32-11.32L96,188.69,218.34,66.34a8,8,0,0,1,11.32,11.32Z" />
              </svg>
              <span className="fs-sm c-slate-8">Unlimited projects</span>
            </li>
            <li className="d-f ai-c g-2">
              <svg
                className="w-4 h-4 c-green f-current"
                viewBox="0 0 256 256"
                role="img"
                aria-label="Check"
              >
                <path d="M229.66,77.66l-128,128a8,8,0,0,1-11.32,0l-56-56a8,8,0,0,1,11.32-11.32L96,188.69,218.34,66.34a8,8,0,0,1,11.32,11.32Z" />
              </svg>
              <span className="fs-sm c-slate-8">Advanced analytics</span>
            </li>
            <li className="d-f ai-c g-2">
              <svg
                className="w-4 h-4 c-green f-current"
                viewBox="0 0 256 256"
                role="img"
                aria-label="Check"
              >
                <path d="M229.66,77.66l-128,128a8,8,0,0,1-11.32,0l-56-56a8,8,0,0,1,11.32-11.32L96,188.69,218.34,66.34a8,8,0,0,1,11.32,11.32Z" />
              </svg>
              <span className="fs-sm c-slate-8">Priority support</span>
            </li>
            <li className="d-f ai-c g-2">
              <svg
                className="w-4 h-4 c-green f-current"
                viewBox="0 0 256 256"
                role="img"
                aria-label="Check"
              >
                <path d="M229.66,77.66l-128,128a8,8,0,0,1-11.32,0l-56-56a8,8,0,0,1,11.32-11.32L96,188.69,218.34,66.34a8,8,0,0,1,11.32,11.32Z" />
              </svg>
              <span className="fs-sm c-slate-8">Custom integrations</span>
            </li>
          </ul>
          <button
            type="button"
            className="mt-auto ai-c d-f jc-c px-4 py-2 br-0 fs-sm fw-500 bg-slate-8 c-white h:bg-slate-10 f:oc-silver-4 f:os-s f:ow-2"
          >
            Get started
          </button>
        </div>
      </div>
    </section>
  );
}
