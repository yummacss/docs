import { CloudUpload } from "iconoir-react";

export default function FileUploadHelper() {
  return (
    <section className="d-f fd-c ai-c g-3 w-100 bg-white bc-silver-2 br-xxl bw-2 bs-d">
      <div className="d-f fd-c ai-c g-2 p-8 bg-white ta-c">
        <div className="d-f ai-c jc-c w-10 h-10 bg-white bc-silver-2 br-lg bw-1">
          <CloudUpload className="w-5 h-5 c-slate-6" />
        </div>
        <div className="d-f fd-c ai-c g-1">
          <span className="c-slate-10 fs-sm fw-500">
            <a className="c-indigo td-none c-p">Upload assets</a> or drag and
            drop
          </span>
          <span className="c-slate-6 fs-xs fw-400">
            Upload project files here
          </span>
        </div>
      </div>
      <p className="m-0 c-slate-6 fs-xs w-100% ta-c pb-6">
        Accepted formats: PNG, JPG, SVG, PDF. Max 10 MB.
      </p>
    </section>
  );
}
