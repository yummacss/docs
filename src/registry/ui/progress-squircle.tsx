...
export default function ProgressSquircle() {
  ...
  return (
    <Progress.Root className="d-f fd-c g-2 w-64" value={value}>
      ...
      <Progress.Track className="o-h h-2 bg-silver-2 br-xxl cs-s">
        <Progress.Indicator
          render={
            <motion.div
              animate={{ width: `${value}%` }}
          ...
          className={(state) =>
            `h-100% br-xxl cs-s ${state.status === "complete" ? "bg-green" : "bg-indigo"}`
          }
        />
      </Progress.Track>
    </Progress.Root>
