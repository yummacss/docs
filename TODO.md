...
- [ ] **Dark theme across every component.** Yumma CSS has handled dark since
      3.29.0, so this is a Yumma UI concern now. Its own dated entry in
      `CHANGELOG.md`, not a `1.0`: see NOTES.md under Versioning. Mockups need a
      theme toggle from the start.
