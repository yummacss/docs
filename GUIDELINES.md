...
- MUST: Visible focus rings (`fv:`; group with `fw:`).
- MUST: Use the `fv:` Yumma CSS utility (e.g., `fv:oc-white fv:ow-2`).
- MUST: Use the `fw:` Yumma CSS utility for `focus-within` styles.
...
- SHOULD: Avoid skeletons unless layout shift becomes significant (user preference)
...
- MUST: `scroll-margin-top` on headings; hierarchical `<h1>`–`<h6>`
...
- SHOULD: Only use non-breaking spaces if strictly necessary for layout stability
